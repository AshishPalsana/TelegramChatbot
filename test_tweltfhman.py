...
REGISTER_PHONE, VERIFY_OTP, LOGIN_USERNAME = range(3)
...
ASK_EMAIL = range(1)
ASK_OTP = range(2)
KYC_VERIFICATION=range(0,10)
Ask_PAN_Details=range(5)
GET_PAN_DOB=range(5)
GET_PAN_NUMBER=range(10)
UPLOAD_PHOTO=range(10)
GET_PAN_NAME=range(10)
GET_PAN_PHOTO=range(10)
# ======TWELFTH APIS=================
...
def ValidateEMail(OTP,context):
    url = "https://fantasy-ci-dev.twelfthman.io/api-qa/v2/VerifyEmail"
...
def GetProfile(context):
    url = "https://fantasy-ci-dev.twelfthman.io/api-qa/v2/GetProfile"
...
def Save_pan(context):
    url = "https://fantasy-ci-dev.twelfthman.io/api-qa/v2/SavePan"

    payload = {'Name': 'KRISHNA BHOWMICK',
    'PanNo': 'AYTPB8437D',
    'DOB': '1986/04/19'}
    files=[
    ('FileName',('KRISHNA_PAN.png',open('/Users/akashpatel/Desktop/KRISHNA_PAN.png','rb'),'image/png'))
    ]
    headers = {
    'Content-Type': 'application/json',
    'Token': '8ec8a4bd0598ec9ef7f5b82367fa0709',
    'Authorization': 'Bearer 9434b9032302dceea9a378d56c167443'
    }
...
def start(update: Update, context: CallbackContext) -> None:
    ...
    if context.user_data.get('is_logged_in', False):
        ...
        return
    Name=user.username if user.username !=None else user.first_name
    if not context.user_data.get('welcome_message_displayed', False):
        update.message.reply_text(f'Hello {Name} . Welcome to Sports Fantasy World. '
...
def select_sport(update: Update, context: CallbackContext) -> int:
    ...
    if selected_sport_id=="recharge_wallet":
        KYC_verification(update,context)
...
def main_manu_hendler(update: Update, context: CallbackContext) -> int:
    ...
    if selected_option == 'upcoming_matches':
        ...
        # isKYCVerified=""
        # if get_balance_api_response['code']==200:
        #     isKYCVerified=get_balance_api_response['data']['isKYCVerified']
        # if not isKYCVerified:
        #     if get_profile_api_response['data']['IsEmailVerified']:
        #         update.callback_query.message.reply_text("Your Email is already Verified")
        #     update.callback_query.message.reply_text("To recharge Wallet, Complete the KYC Process")
        #     update.callback_query.message.reply_text("Please provide your email address.")
        #     # validate_email(update,context)
        #     return ASK_EMAIL
        
def KYC_verification(update: Update, context: CallbackContext):
    # get_balance_api_response=GetBalance(context)
    get_profile_api_response=GetProfile(context)
    print("KYC VERIFICATION")
    isEmailVerified=get_profile_api_response['data']['IsEmailVerified']
    isPanVerified=get_profile_api_response['data']['IsPanVerified']
    isBankVerified=get_profile_api_response['data']['IsBankVerified']
    
    if isEmailVerified and isPanVerified and isBankVerified == 1:

         # All verifications passed, allow wallet recharge
        update.message.reply_text("KYC is verified. You can recharge your wallet.")
        return "Recharge Wallet"
    else:

        if isEmailVerified==0:
            update.message.reply_text("Please verify your email first.")
            update.callback_query.message.reply_text("Please provide your email address.")
            return ASK_EMAIL
        elif not isPanVerified==0:
            update.callback_query.message.reply_text("Please provide your PAN card details.")
            ask_pan_details(update,context)
            return Ask_PAN_Details
            
        # elif not isBankVerified==0:
        #     update.message.reply_text("Please provide your bank details.")
        #     return "Ask Bank Details"
    
    
def validate_email(update,context):
    query = update.callback_query
    selected_option = query.data
    print("Validate EMAIL MODULE")
    context.user_data['recharge_email'] = selected_option

    update.message.reply_text(f"Thank you! You provided the email: {selected_option}")
    update.message.reply_text("Please enter the OTP sent to your email.")
    return ASK_OTP
def save_otp(update, context):
    query = update.callback_query
    selected_option = query.data
    print("Validate EMAIL OTP MODULE")
    api_response= ValidateEMail(selected_option)

def ask_pan_details(update: Update, context: CallbackContext):
    
    update.effective_message.reply_text("1. Name as per PAN card")
    return GET_PAN_NAME
def get_pan_name(update: Update, context: CallbackContext):
    pan_name = update.message.text
    print(pan_name)
    context.user_data['pan_name'] = pan_name
    # update.callback_query.message.reply_text(f"Thank you! You provided the PAN name: {pan_name}")
    # update.callback_query.message.reply_text("2. 10-digit PAN number")
    if update.message:
                update.message.reply_text("2. 10-digit PAN number")
    
    return GET_PAN_NUMBER

def get_pan_number(update: Update, context: CallbackContext):
    pan_number = update.message.text
    print("====",pan_number)
    context.user_data['pan_number'] = pan_number
    # update.callback_query.message.reply_text("3. Date of birth as per PAN card (YYYY-MM-DD)")
    if update.message:
                update.message.reply_text("3. Date of birth as per PAN card (YYYY-MM-DD)")
    else:
                context.bot.send_message(
                    chat_id=update.effective_chat.id, text="3. Date of birth as per PAN card (YYYY-MM-DD)")
    return GET_PAN_DOB

def get_pan_dob(update: Update, context: CallbackContext):
    pan_dob = update.message.text
    
    context.user_data['pan_dob'] = pan_dob
    # update.message.reply_text(f"Thank you! You provided the PAN DOB: {pan_dob}")
    update.message.reply_text("4. Upload PAN card photo ")
    return "Upload PAN Photo"

def upload_pan_photo(update: Update, context: CallbackContext):
    # Handle the photo upload logic if needed
    update.message.reply_text("PAN card details received successfully!")
    print(context.user_data)
def get_pan_photo(update: Update, context: CallbackContext):
    # Assuming you want to store the photo in context
    photo_file = update.message.photo[-1].get_file()
    context.user_data['pan_photo'] = photo_file.download_as_bytearray()

# ==============
def create_team(update: Update, context: CallbackContext):
    query = update.callback_query
    selected_option = query.data
    print("CREATE TEAM MODULE")
        
    response_data = GetMatchPlayer(context.user_data['GameGroupID'], context)
...
def main() -> None:
    ...
    conv_handler = ConversationHandler(
        ...
        states={
            ...
            KYC_VERIFICATION: [CallbackQueryHandler(KYC_verification)],
            ASK_EMAIL: [MessageHandler(Filters.text & ~Filters.command, validate_email)],
            Ask_PAN_Details: [MessageHandler(Filters.text & ~Filters.command, ask_pan_details)],
            # GET_PAN_NAME: [MessageHandler(Filters.text & ~Filters.command, get_pan_name)],
            GET_PAN_NAME: [MessageHandler(Filters.text & ~Filters.command, get_pan_name)],
            GET_PAN_NUMBER: [MessageHandler(Filters.text & ~Filters.command, get_pan_number)],
            GET_PAN_DOB: [MessageHandler(Filters.text & ~Filters.command, get_pan_dob)],
            UPLOAD_PHOTO: [MessageHandler(Filters.photo, upload_pan_photo)],
            GET_PAN_PHOTO: [MessageHandler(Filters.photo, get_pan_photo)],
